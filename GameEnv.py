...
GOALREWARD = 25   #if goal is reached then reward=1
LIFE_REWARD = 0  #if its stays alive then reward=0
PENALTY = -10     #if penalty then reward=-1

def distance(pt1, pt2): #used to calculate distance
    return(((pt1.x - pt2.x)**2 + (pt1.y - pt2.y)**2)**0.5)

def rotate(origin,point,angle): #used to rotate car by a certain angle
...
def rotateRect(pt1, pt2, pt3, pt4, angle): #used to rotate the rectangle which consist the car
...
class Ray:
    #used to define the rays around the car(or object)
...
class Car:
    #used to define car and its action
    def __init__(self, x, y):
        self.pt = myPoint(x, y)
        self.x = x
        self.y = y
...
class RacingEnv:
    ...
    def __init__(self):
        ...
        self.back_image = pygame.image.load("map0.png").convert()
    ...
    def render(self, action):
        ...
        # score
        text_surface = self.font.render(f'Points scored in the current episode: {self.car.points}', True, pygame.Color('green'))
